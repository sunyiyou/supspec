...
def main(log_writer, log_file, device, args):
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(args.dataset_root, TransformTwice(transform_train)),
        batch_size=args.train.batch_size, shuffle=True,
        num_workers=num_workers, pin_memory=True)

    test_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(args.dataset_root, transform_test),
        batch_size=args.eval.batch_size, shuffle=True,
        num_workers=num_workers, pin_memory=True)
    ...
    for epoch in range(0, args.train.stop_at_epoch):
        ...
        for idx, ((view1, view2), combined_target) in enumerate(train_loader):
            ...
            model.zero_grad()
            data_dict = model(x1, x2, ux1, ux2, None, mu=args.model.mu)
        ...
        if (epoch + 1) % args.deep_eval_freq == 0:
            ...
            features_test_u, ltest_u, _ = feat_extract(test_loader, proto_type='all')
            ...
            centroids[0, :] = ftest_u[0]
            centroids = torch.Tensor(centroids).cuda()
            # K-Means++ initialization
            for icls in range(1, centroids_size):
            ...
            seen_acc = cluster_acc(preds[seen_mask], ltest_u[seen_mask])
...
def get_args():
    ...
    parser.add_argument('-c', '--config-file', default='configs_openncd/spectral_resnet50_mlp8192_imagenet100.yaml', type=str)
    parser.add_argument('--debug', action='store_true')
    parser.add_argument('--log_freq', type=int, default=100)
    parser.add_argument('--workers', type=int, default=4)
    parser.add_argument('--test_bs', type=int, default=80)
    parser.add_argument('--download', action='store_true', help="if can't find dataset, download from web")
    parser.add_argument('--dataset_root', default='/home/sunyiyou/dataset/imagenet/train100', type=str)
    ...
    disc = f"-lr{args.base_lr}-seed{args.seed}"

    ...
    for epoch in range(0, args.train.stop_at_epoch):
        ...
        if (epoch + 1) % args.deep_eval_freq == 0:
            ...
            if (epoch + 1) % args.vis_freq == 0:
                ...
                tvis = ltest_u[rand_idx].astype(int)

                os.makedirs(os.path.join(args.log_dir, 'vis'), exist_ok=True)
                colors = ["#cc4400","#d66915","#e08e29","#f0c761","#ffff99","#c2fcff","#7cc6de","#3890bc","#1c489a","#000077"]
...
# python pretrain_openncd.py --layer 'proj'
# python pretrain_openncd.py --layer 'penul'
...
def get_args():
    ...
    parser.add_argument('--gamma_l', default=0.5, type=float)
